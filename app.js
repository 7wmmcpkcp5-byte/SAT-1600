...
const EXAM_TOTAL_SECONDS = 45 * 60; // 45 minutes
...
function selectAdaptiveLevels(){
  ...
  ['1','2','3'].forEach(lvl => {
    ...
    if (!v || !v.total){
      arr.push(lvl); // not practiced yet -> focus
      return;
    }
    const p = v.correct / v.total;
    if (p < 0.8) arr.push(lvl); // weaker level
  });
...
function startQuiz(mode){
  ...
  if (mode === 'exam'){
    // exam uses all levels for that subject
    levels = ['1','2','3'];
  }
...
function registerCorrect(q){
  if (!STATE.currentUser || !q) return;
  // nothing extra for now (stats done in updateStatsForQuestion)
}
