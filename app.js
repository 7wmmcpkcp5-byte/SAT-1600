...
async function loadQuestions(section, level) {
  ...
  if (questionCache[section][level]) return questionCache[section][level];

  const file = `${section}_level${level}.json`;
  const res = await fetch(file);
  if (!res.ok) throw new Error("Cannot load " + file);
  const data = await res.json();
  // Data is list of {q, opts, correct}
  const normalized = data.map((item, index) => ({
    id: `${section}_${level}_${index}`,
    text: item.q,
    options: item.opts,
    correctIndex: item.correct
  }));
  questionCache[section][level] = normalized;
  return normalized;
}
